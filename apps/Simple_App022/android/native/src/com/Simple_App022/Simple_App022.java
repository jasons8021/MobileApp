...
/*			task.post(new Runnable() {
			...
			task.post(new Runnable() {
				...
				@Override
				public void run() {
					Simple_App022.this.bindService(new Intent(Simple_App022.this,
		...
		task.post(new Runnable() {
			...
			public void run() {
				...
				try {
					SharedPreferences sp = Simple_App022.this
					...
					if (StringUtils.isNotBlank(message)) {
						Simple_App022.this.callMessageHandlerRouters(message);
					}
					SharedPreferences.Editor spEditor = sp.edit();
	...
	public void onPause() {
		...
		if (mqsConnected) {
			task.post(new Runnable() {
				...
				@Override
				public void run() {
					Simple_App022.this.unbindService(conn);
				}
			});
	...
	private void loadResource() {
		...
		// @Override
		// public void run() {
		MQTTResources.init(Simple_App022.this);
		// }
		// });
	}
	...
	private class MQServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			Simple_App022.this.mqs = ((MQBinder) service).getService();
			if (Simple_App022.this.clientId != null
					&& Simple_App022.this.topic != null) {
				Simple_App022.this.task.post(Simple_App022.this.connectMQ);
			}
			Simple_App022.this.mqsConnected = true;
		}

		@Override
		public void onServiceDisconnected(ComponentName name) {
			Simple_App022.this.mqsConnected = false;
		}
	...
	private class MQConnectTask implements Runnable {
		...
		@Override
		public void run() {
			SharedPreferences sp = Simple_App022.this.getSharedPreferences(
					"PersistenceSharedPreferences", Context.MODE_PRIVATE);
			SharedPreferences.Editor spEditor = sp.edit();
			spEditor.putString("clientId", clientId).commit();
			spEditor.putString("topic", topic).commit();
			if (Simple_App022.this.mqs.connect(clientId, topic)) {
				task.post(new Runnable() {
					@Override
					public void run() {
						NotificationManager mNotifMan = MQTTResources
	...
	public void publishMessage(final String topic, final String message,
		...
		task.post(new Runnable() {
			...
			@Override
			public void run() {
				Simple_App022.this.mqs.publishMessage(topic, message, qos);
			}
		});
	}
